...
NumericVector lengthx_cpp1(int dleft, int dright, IntegerVector L_V4_1, IntegerVector L_V6m_1, NumericVector L_V4_2, IntegerVector L_V6m_2,IntegerVector R_V4_1, IntegerVector R_V6m_1, NumericVector R_V4_2, IntegerVector R_V6m_2, IntegerVector L_V6t, IntegerVector R_V6t){
  ...
  for(int i = 0;i <n;i++)
  {
    ...
    if(  ( dleft< (L_V4_1[i]+L_V6m_1 [i]-2)) & (L_V4_1[i]<dright) ){
      x[i]=1;
    }
    else if( (!NumericVector::is_na(L_V4_2[i])) &( dleft< (L_V4_2[i]+L_V6m_2 [i]-2)) & (L_V4_2[i]< dright)){
      x[i]=1;
    }
    else if (( dleft< (R_V4_1[i]+R_V6m_1 [i]-2)) & (R_V4_1[i]<dright)  ){
      x[i]=1;
    }
    else if ((!NumericVector::is_na(R_V4_2[i]))&( (dleft) < (R_V4_2[i]+R_V6m_2 [i]-2)) & (R_V4_2[i]<dright)){
      x[i]=1;
    }
//#####not supply but split########
   else if ( (NumericVector::is_na(L_V4_2[i])) &(L_V6m_1[i]!=L_V6t[i])&(dright<L_V4_1 [i])&(L_V4_1[i]<dright+3)&((dright-1)<R_V4_1 [i])&(R_V4_1[i]<(dright+3))) {
       x[i]=R_V4_1[i]-L_V4_1[i]+R_V6t[i];
         }
    else if ( (NumericVector::is_na(L_V4_2[i])) &(L_V6m_1[i]!=L_V6t[i])&(dright<L_V4_1 [i])&(L_V4_1[i]<(dright+3))) {
      x[i]=R_V4_1[i]-L_V4_1[i]+R_V6t[i]+(L_V6t[i]-L_V6m_1[i]);
    }
   else if ( (NumericVector::is_na(R_V4_2[i])) &(R_V6m_1[i]!=R_V6t[i]) & (dright<R_V4_1[i])&(R_V4_1[i]<dright+3)&((dright-1)<L_V4_1 [i])&(L_V4_1[i]<(dright+3))) {
       x[i]=R_V4_1[i]-L_V4_1[i]+R_V6t[i];
   }
    else if( (NumericVector::is_na(R_V4_2[i])) &(R_V6m_1[i]!=R_V6t[i]) & (dright<R_V4_1[i])&(R_V4_1[i]<(dright+3))){
      x[i]=R_V4_1[i]-L_V4_1[i]-(dright-dleft+1)+R_V6m_1[i];
    }
    
    else if(L_V4_1[i]<=(dleft)&R_V4_1[i]>=dright){
...
//#####start prob =0#########
    ...
    else if(  ( dleft < (L_V4_1[i]+L_V6m_1[i]-2)) & (L_V4_1[i]<dright) ){
      y[i]=0;
    }
    else if( (!NumericVector::is_na(L_V4_2[i]))&( dleft < (L_V4_2[i]+L_V6m_2[i]-2)) & (L_V4_2[i]< dright)){
      y[i]=0;
